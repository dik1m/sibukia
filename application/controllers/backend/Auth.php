...
class Auth extends CI_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        //Load model
        $this->load->model('backend/M_Auth');
    ...
    public function index()
    {
        ...
        $this->form_validation->set_rules('pswd_user', 'Password', 'trim|required');
    ...
    private function _login()
    {
        ...
        $pswd    = md5($this->input->post('pswd_user'));
        $verify = $this->M_Auth->CekLogin('user', $email, $pswd);
        ...
        if ($user) {
            ...
            if ($user['active_user'] == 1) {
                //jika login
                //if (password_verify($pswd, $user['pswd_user'])) {
                if ($verify > 0) {
                    $data = array(
                    ...
                    $this->session->set_flashdata('message_true', 'hello.. ' . $this->session->userdata('nm_user') . ' Selamat Datang dihalaman useristartor');
            ...
            $this->session->set_flashdata('message_false', '<center>Login Gagal!<br>Email tidak tersedia</center>');
            redirect($this->redirect, 'refresh');
        }
    }
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'bangsribrebes2021@gmail.com',
            'smtp_pass' => 'Bangsri2021',
            'smtp_port' => 465,
            'mailtype'  => 'html',
            'charset'   => 'utf-8',
            'newline'   => "\r\n"
        ...
        $this->email->from('bangsribrebes2021@gmail.com', 'Kelurahan Desa Bangsri');
        $this->email->to($this->input->post('email_user'));

        if ($type == 'verify') {
            $this->email->subject('Account Verification');
            $this->email->message('Click this link to verify you account : <a href="' . base_url() . 'backend/Auth/verify?email=' . $this->input->post('email_user') . '&token=' . urlencode($token) . '">Activate</a>');
        } else if ($type == 'forgot') {
            $this->email->subject('Reset Password');
            $this->email->message('Klik link ini untuk reset password : <a href="' . base_url() . 'index.php/backend/Auth/resetpassword?email=' . $this->input->post('email_user') . '&token=' . urlencode($token) . '">Reset Password</a>');
        }
    ...
    public function changePassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->db->set('pswd_user', $password);
